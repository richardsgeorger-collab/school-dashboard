...
| Class pass | `plan.ts` | One forced tool call (`class_plan`) per class. Per item: what it asks for, a reasoned start-by, effort in minutes, milestones from the rubric, prerequisites (with the source, including announcements), flags, topics, the material that covers it, citations, and a confidence per field. Also work found outside Halo (needs a quote; no written date means no due date) and the class's topic map (what builds on what). |
...
| --- | --- | --- | --- | --- |
| Class pass, one class | 25–50k | 8–14k | $0.20–0.35 | Syllabus + 30 descriptions + rubrics + deck outlines. ~1 min. |
