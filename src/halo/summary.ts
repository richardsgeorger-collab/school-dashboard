...
export interface SummaryFinding {
  ...
  date: string | null;
  /** It gates other work, so it counts as something to act on even though it is an addition. */
  gating?: boolean;
}
...
/** Grades and plain additions are housekeeping; these are the kinds that need attention. */
const PROBLEM: Proposal['kind'][] = ['update', 'remove', 'flag'];
const isProblem = (f: SummaryFinding) => PROBLEM.includes(f.kind) || (f.kind === 'add' && f.gating) || f.status === 'overdue' || f.status === 'missing' || f.status === 'schedule';
...
export function localSummary(input: SummaryInput): AuditSummary {
  ...
  const problems = real.filter(isProblem);
  const grades = real.filter((f) => f.kind === 'score' || f.status === 'grade');
  const adds = real.filter((f) => f.kind === 'add' && !f.gating);
  const byClass = new Map<string, number>();
  for (const f of problems) byClass.set(f.classCode, (byClass.get(f.classCode) ?? 0) + 1);
  const top = [...byClass.entries()].sort((a, b) => b[1] - a[1])[0];
  const where = top && byClass.size > 1 && top[1] > 1 ? `, ${top[1]} of them in ${top[0]}` : top && byClass.size === 1 && problems.length > 1 ? `, all in ${top[0]}` : '';
  const bulkLines = (input.bulk ?? []).map((b) => `${b.code} isn't in your planner yet — ${b.ids.length} item${b.ids.length === 1 ? '' : 's'} to add.`);
  const housekeeping = [grades.length ? `${grades.length} grade${grades.length === 1 ? '' : 's'}` : '', adds.length ? `${adds.length} new item${adds.length === 1 ? '' : 's'}` : ''].filter(Boolean);
  const tail = housekeeping.length ? ` ${housekeeping.join(' and ')} come${grades.length + adds.length === 1 ? 's' : ''} over on ${grades.length + adds.length === 1 ? 'its' : 'their'} own.` : '';
  let verdict: string;
  ...
  else if (problems.length === 0) verdict = `Nothing needs fixing.${tail}`;
  else verdict = `${problems.length <= 2 ? 'Mostly clean' : problems.length <= 5 ? 'A few things need attention' : 'Quite a bit needs attention'} — ${problems.length} thing${problems.length === 1 ? '' : 's'} to act on${where}.${tail}`;
  const matters = [
    ...bulkLines,
    ...[...problems, ...real.filter((f) => !isProblem(f))]
      .sort((a, b) => Number(isProblem(b)) - Number(isProblem(a)) || (a.date ?? '9999').localeCompare(b.date ?? '9999'))
  ...
  const partial = input.classes.filter((c) => c.outcome === 'partial').map((c) => `${c.code} wasn't fully checked: ${lower(c.reason)}${c.skipped.length ? ` Not checked: ${c.skipped.join('; ')}.` : ''} Don't trust its result yet.`);
  const unreached = input.classes.filter((c) => c.outcome === 'not reached').map((c) => c.code);
...
- verdict: one sentence. Count only what needs action: moved deadlines, missing items, gating deadlines, late flags, schedule changes. Posted grades and plain new items are housekeeping, not problems; mention them in a short second clause ("7 grades and 4 new items come over on their own"). Name classes by their full code (ESG-162, ENG-105), never a nickname.
...
export function buildSummaryPrompt(input: SummaryInput): { system: string; user: string } {
  const findings = input.findings.length
    ? input.findings.map((f) => `${f.id} · ${f.classCode} · ${f.status} · "${f.title}"${f.date ? ` · ${f.date}` : ''} · ${f.confidence} confidence${f.gating ? ' · gates other work' : ''}\n   Halo said: ${f.quote}\n   Proposal (${f.kind}): ${f.proposal}`).join('\n')
    : '(none)';
  const classes = input.classes.map((c) => `${c.code}: ${c.outcome} — ${c.reason}${c.skipped.length ? ` Skipped: ${c.skipped.join('; ')}` : ''}`).join('\n');
