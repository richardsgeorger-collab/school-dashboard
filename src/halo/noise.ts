...
/** Something a finding line would carry; narration that carries it is kept as possible content. */
const CONTENT = /\d{1,2}\/\d{1,2}|\d{4}-\d{2}-\d{2}|\b(due|pts|points|late|missing|overdue|changed|graded?|score)\b/i;
...
export const isNoiseLine = (line: string): boolean => {
  ...
  return NARRATION.test(l) && !CONTENT.test(l);
};
