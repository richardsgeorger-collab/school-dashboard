...
const STRONG = /^(used claude in chrome|moving to (class|the last)|opened attachments|read all \d+ announcements|all \d+ planner items|every dq|the \d+ findings above|\d+ rows\.|one more thing|i have to stop you|this session hit|compaction replaced|that means i can|two honest options|scroll up|say the word|i'd lean|no (new|other) (items|findings|differences|due-date)|nothing else\b|moving (on|to)\b|starting phase|that('s| is) the last|opened halo|your planner (has|had) no items|the (one|two|three|things?) that (actually )?matters?|everything else,?|compiling|summary of (what|findings)|here('s| is) (what|the))/i;
...
export const CONTENT = /\b\d{1,2}\/\d{1,2}(\/\d{2,4})?\b|\d{4}-\d{2}-\d{2}|\b(jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\.?\s+\d{1,2}\b|\b\d+(\.\d+)?\s*(pts?|points?)\b|\b\d+(\.\d+)?\s*\/\s*\d+(\.\d+)?\b|\b(due|overdue|late|missing|unsubmitted)\b/i;
...
export const isCoverageLine = (line: string): boolean => /\bvisited\s+\d+\s+(of|\/)\s+\d+/i.test(line) || /^\(?(coverage|visited|stopped|final coverage|all match|end of findings|skipped|skipping|not visited)\b/i.test(line.trim()) || (/\bskipp(ed|ing)\b/i.test(line) && /\b(generic|institution resources|university-wide|exclusion list)\b/i.test(line)) || /^\(?no page failed/i.test(line.trim());
