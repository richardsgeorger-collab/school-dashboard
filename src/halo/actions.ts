...
export async function readActions(args: { apiKey: string; fetch?: typeof globalThis.fetch; announcement: StoredAnnouncement; course: Course; items: Item[]; tz: string }): Promise<{ summary: string; actions: Action[] }> {
  ...
  return actionsFromTool(r.input, args.announcement, args.items, args.tz);
}
