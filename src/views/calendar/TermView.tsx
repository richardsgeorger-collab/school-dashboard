...
export function TermView({ items, onOpen }: { items: Item[]; onOpen: (i: Item) => void }) {
  ...
  return (
    <div className="term">
      ...
      {/* One heaviest week, named. The four-week rundown was a block nobody reads. */}
      {(() => {
        const next = (data.settings.termPlan?.weeks ?? []).filter((w) => w.start >= today && (w.load === 'brutal' || w.load === 'heavy') && w.why).slice(0, 1)[0];
        if (!next) return null;
        return (
          <p className="hint">
            <span className="ai-from">AI</span> Your hardest week ahead starts {fmtDate(next.start, 'short')}: {next.why}
          </p>
        );
      })()}
