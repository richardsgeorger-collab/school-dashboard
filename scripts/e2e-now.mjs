...
page.on('request', (req) => {
  ...
  if (tool === 'lecture_notes') {
    ...
    return req.respond({ status: 200, headers: { ...cors, 'content-type': 'application/json' }, body: JSON.stringify({ id: 'm', type: 'message', role: 'assistant', model: 'claude-sonnet-4-6', content: [{ type: 'tool_use', id: 'tu', name: 'lecture_notes', input }], stop_reason: 'tool_use', usage: { input_tokens: 3000, output_tokens: 400 } }) });
  }
  if (tool === 'text') return req.respond({ status: 200, headers: { ...cors, 'content-type': 'application/json' }, body: JSON.stringify({ id: 'm', type: 'message', role: 'assistant', model: 'claude-sonnet-4-6', content: [{ type: 'text', text: 'Start by picking one artifact from the topic list. Which one interests you? [S1]' }], stop_reason: 'end_turn', usage: { input_tokens: 2000, output_tokens: 40 } }) });
